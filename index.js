function createRow(info) {
    ...
    for (let i = 0; i < $columns.length; i++) {
        ...
        if (id === 'name') {
            ...
            if (info.difficulty === 'easy') $td.setAttribute('class', 'text-success');
            if (info.difficulty === 'intermediate') $td.setAttribute('class', 'text-warning');
            if (info.difficulty === 'hard') $td.setAttribute('hard', 'text-danger');
        } else if (id === 'date') {
            ...
            if (!info.completedOnTime) $bold.setAttribute('class', 'text-danger');
            if (info.completedOnTime) $bold.setAttribute('class', 'text-success');
            $td.appendChild($bold);
        } else if (id === 'complete') {
